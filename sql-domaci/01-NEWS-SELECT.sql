...
FROM
    `news`
WHERE
    `portal` IN ('Blic', 'Kurir', 'Novosti') AND `category` = 'Politika';
...
FROM
    `news`
WHERE
    `portal` IN ('Blic', 'Kurir', 'Novosti') AND `category` IN ('Politika', 'Sport', 'Srbija');
...
FROM
    `news`
WHERE
    `on_homepage` = 1 AND `facebook_likes` > 100;
...
FROM
    `news`
WHERE
    `on_homepage` = 1 AND `publish_time` > '2017-10-24' AND `publish_time` < '2017-10-26';
